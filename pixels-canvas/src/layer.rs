...
impl From<Image> for Layer{
    fn from(value: Image) -> Self {
        ...
        let layer_element = LayerElement { position: (0, 0), image: value, };
        Layer { elements: vec![layer_element], size }
    }
}

impl Layer{
    pub fn new(size: (u64, u64)) -> Self{
        Layer { elements: Vec::new(), size }
    }
    ...
    pub fn create_layer_element(&mut self, position: (u64, u64), image: Image){
...
impl LayerElement{
    ...
    pub fn set_raw_data(&mut self, raw_data: Vec<u8>, size: (u64 , u64)){
        self.image = Image::from_vec(raw_data, size);
    }
